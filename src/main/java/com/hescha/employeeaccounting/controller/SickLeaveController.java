...
@RequestMapping(SickLeaveController.CURRENT_ADDRESS)
public class SickLeaveController {
    public static final String API_PATH = "sickleave";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new SickLeave());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute SickLeave entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                SickLeave createdEntity = service.create(entity);

...
@RequestMapping(VacationController.CURRENT_ADDRESS)
public class VacationController {
    public static final String API_PATH = "vacation";
    ...
    public String editPage(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id == null) {
            model.addAttribute("entity", new Vacation());
        } else {
            model.addAttribute("entity", service.read(id));
        }
    ...
    @PostMapping
    public String save(@ModelAttribute Vacation entity, RedirectAttributes ra) {
        if (entity.getId() == null) {
            try {
                Vacation createdEntity = service.create(entity);
